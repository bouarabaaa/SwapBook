...
include "connexion.php";
    ...
    if(!empty($_GET["date"])){
      $gt='date';
        ...
        $reqt = "select * from livre2 where iduser in (select id from user where typee = 'bibliotheque') and EXTRACT(YEAR FROM datedition) ='$d' LIMIT $start, $limit";
        $reqttcn = "SELECT count(idlivree) from livre2 where iduser in (select id from user where typee = 'bibliotheque') and EXTRACT(YEAR FROM datedition) ='$d' ";
    ...
    elseif(!empty($_GET["ville"])){
      $gt='ville';
        ...
        $reqt = "select * from livre2 where iduser in (select id from user where typee = 'bibliotheque' and ville='$d')LIMIT $start, $limit";
        $reqttcn = "SELECT count(idlivree) from livre2 where iduser in (select id from user where typee = 'bibliotheque' and ville='$d')";
    ...
    elseif(!empty($_GET["cat"])){
      $gt='cat';
        ...
        $reqt = "select * from livre2 where iduser in (select id from user where typee = 'bibliotheque') and categorie ='$d' LIMIT $start, $limit";
        $reqttcn = "SELECT count(idlivree) from livre2 where iduser in (select id from user where typee = 'bibliotheque') and categorie ='$d'";
      ...
      $d=$_GET["pt"];
        ...
        $reqt = "select * from livre2 where iduser in (select id from user where typee = 'bibliotheque') and pointe ='$d' LIMIT $start, $limit";
        $reqttcn = "SELECT count(idlivree) from livre2 where iduser in (select id from user where typee = 'bibliotheque') and pointe ='$d'";
...
<div class="d-flex justify-content-center m-5">
  ...
  <ul class="pagination">
    <li class="page-item ">
      <a class="page-link" href="recherchavcbib.php?<?= $gt; ?>=<?= $d; ?>&page=<?= $Previous; ?>" tabindex="-1">Previous</a>
    </li>
    ...
    <?php for($i = 1; $i<= $pages; $i++) :
     ...
     ?>
    <li class="page-item <?php echo $pgn; ?>" ><a class="page-link" href="recherchavcbib.php?<?= $gt; ?>=<?= $d; ?>&page=<?= $i; ?>"><?= $i; ?></a></li>

    <?php endfor; ?> 

    <li class="page-item">
      <a class="page-link" href="recherchavcbib.php?<?= $gt; ?>=<?= $d; ?>&page=<?= $Next; ?>" tabindex="-1">Next</a>
    </li>
  </ul>
</nav>
</div>
