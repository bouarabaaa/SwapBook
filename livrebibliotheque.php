...
<html>
<head>
  <title>Livre Bibliotheque</title>
  <meta charset="utf-8"/>
...
include "connexion.php";
    ...
    $reqt = "select * from livre2 where iduser in (select id from user where typee = 'bibliotheque') LIMIT $start, $limit";
    $reqttcn = "SELECT count(idlivree) from livre2 where iduser in (select id from user where typee = 'bibliotheque') ";
...
if(!$res || !$ress){
  ...
  $nb = $ress->num_rows;
  $ligneres=mysqli_fetch_array($ress);
 }
...
$reqt8 = "select distinct EXTRACT(YEAR FROM datedition) from livre2 where iduser in (select id from user where typee = 'bibliotheque')";
...
if(!$res8){
 ...
 $reqt9 = "select distinct categorie from livre2 where iduser in (select id from user where typee = 'bibliotheque')";
...
if(!$res9){
 ...
 $reqt10 = "select distinct pointe from livre2 where iduser in (select id from user where typee = 'bibliotheque')";
 ...
 if(!$res10){
  ...
  $reqt11 = "select distinct ville from user where typee='bibliotheque'";
...
?>
      ...
      <div class="text-center mb-5">
        <h1> <span data-text="BIBLIOTHEQUE" class="grtitre">BIBLIOTHEQUE</span> </h1>
    </div>
...
while($ligneres=mysqli_fetch_array($ress)){
...
<!--jai fait ca pour separer les deux container -->
  ...
  if($res8->num_rows>0){
    ...
    while($lign8=mysqli_fetch_array($res8)){
      echo'<a class="dropdown-item" href="recherchavcbib.php?date='.$lign8[0].'">'.$lign8[0].'</a>';
  ...
  if($res11->num_rows>0){
       ...
       while($lign11=mysqli_fetch_array($res11)){
        echo'<a class="dropdown-item" href="recherchavcbib.php?ville='.$lign11[0].'">'.$lign11[0].'</a>';
  ...
  if($res9->num_rows>0){
       ...
       while($lign9=mysqli_fetch_array($res9)){
        echo'<a class="dropdown-item" href="recherchavcbib.php?cat='.$lign9[0].'">'.$lign9[0].'</a>';
...
<?php
  if($res10->num_rows>0){
       ...
       while($lign10=mysqli_fetch_array($res10)){
        echo'<a class="dropdown-item" href="recherchavcbib.php?pt='.$lign10[0].'">'.$lign10[0].'</a>';
...
<div class="d-flex justify-content-center m-5">
  ...
  <ul class="pagination">
    <li class="page-item ">
      <a class="page-link" href="livrebibliotheque.php?page=<?= $Previous; ?>" tabindex="-1">Previous</a>
    </li>
    ...
    <?php for($i = 1; $i<= $pages; $i++) :
     ...
     ?>
    <li class="page-item <?php echo $pgn; ?>" ><a class="page-link" href="livrebibliotheque.php?page=<?= $i; ?>"><?= $i; ?></a></li>

    <?php endfor; ?> 

    <li class="page-item">
      <a class="page-link" href="livrebibliotheque.php?page=<?= $Next; ?>" tabindex="-1">Next</a>
    </li>
  </ul>
</nav>
</div>
